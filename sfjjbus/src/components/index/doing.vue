<template>
	<div class="redoing">
		...
		<div class="top_re_box">
			<div class="container">
				<p style="padding: 14px 0 8px 0px;">快递单号 {{info.id}}</p>
		...
		<div class="active_1" v-for="n in info.info">
       		<div class="ma">
	       		...
	       		<div class="ac_bottom">
	       			<p style="color:rgba(153,153,153,1);">共{{n.num}}件商品 <span style="display: inline-block;margin-left: 16px;color: black;">小记</span> <span style="margin-left: 4px;display: inline-block;color: rgba(208,2,27,1);">¥{{n.price*n.num}}</span></p>
...
//           console.log(_self.$route.query.id)
        this.$store.commit('changetitle', '进行中');
        //修改头部返回箭头
        this.$store.commit('changeBack', true);
        var time   
         time = JSON.stringify(Date.parse(new Date()))
         var data= 'id=' + _self.$route.query.info + '|' +'timestamp=' + time.substring(0,10)
          server.order_detail(_self,data)
        },
      filters: {
