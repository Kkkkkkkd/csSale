			...
			<div class="ma">
				...
				<div class="ac_center1">
					...
					<div class="div_bar">
						...
						<p style="font-size: 20px;margin-top: 5px;color: #D0021B;width: 100%;position: absolute;bottom: 10px;">¥{{n.price}} 
							<!--<span style="font-size: 16px;color: #999999;text-decoration: line-through;">¥72.20</span>-->
							<button style="color:white;float: right;background-color: rgba(235,200,111,1);height:25px;width:56px;float: right;">马上抢</button></p>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		mounted: function() {
			...
			//系统公告请求
			//修改title
			this.$store.commit('changetitle', _self.$route.query.text);
			//修改头部返回箭头
			this.$store.commit('changeBack', true);
			server.goods_list(_self,{
				type:_self.$route.query.id,
				page:'1',
				limit:_self.num,
				upper:'1'
			})
