<template>
	<div class="suredd">
			...
			<div class="ma">
				...
				<div class="ac_center1" style="border-bottom: 1px solid gainsboro;">
					...
					<div class="div_bar" style="position: relative;">
						<span class="s_p">{{info.name}} &nbsp;{{this.$route.query.size}}</span>

						<p style="font-size: 20px;color: #333333;position: absolute;bottom: 12px;width: 100%;">¥{{info.price}} <span style="color:#999999;float: right;">x{{this.$route.query.num}}</span></p>
					</div>

				</div>
				<div class="" style="height: 48px;line-height: 48px;">
					<p style="float: right;font-size: 14px;color: #999999;">共{{this.$route.query.num}}件商品<span style="color: #000000;margin-left: 16px;">小记</span><span style="color: #D0021B;margin-left: 16px;">¥{{info.price*this.$route.query.num}}</span></p>
				</div>
			</div>
		</div>
		...
		<div class="footer_box">
			<span style="margin-left: 12px;">总计金额</span><span style="font-size: 24px;color: #D0021B;margin-left: 12px;">{{info.price*this.$route.query.num-youmy}}元</span>
	...
	export default {
		...
		methods: {
			...
			sure(){
				...
				console.log(this.info2.address)
