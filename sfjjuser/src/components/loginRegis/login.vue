<template>
    <div class="login">
        <div class="con">
            ...
            <div class="box_p">
            	<label for="phone" class="label_p">手机号<input type="number"  placeholder="请填写手机号" name="phone" v-model="phone"/></label>
            	
            </div>
            <div class="box_p" style="margin-top: 0;">
            	<label for="pas" class="label_p">验证码<input type="number"  placeholder="请填写验证码" name="pas" v-model="pas" style="width: 50%;"/><span style="float: right;color:rgba(235,200,111,1);" @click="getyzm" class="yzm">获取验证码</span></label>
...
export default {
    ...
    methods: {
        ...
        login: function () {
            ...
            console.log(!this.phone.match(myreg))
            var va = $('input[name=phone]').val()
          if (this.phone == '') {
            errMsg('请输入手机号');
          } else if (!this.phone.match(myreg)) {
            errMsg('请输入正确的手机号');
          } else if (this.pas == '') {
            errMsg('验证码不能为空');
          } else {
