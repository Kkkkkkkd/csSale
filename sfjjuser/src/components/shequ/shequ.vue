<template>
	<div class="shequ">
		<headCom></headCom>
		<div class="tab_box">
			<div id="" style="z-index: 5555;">
				<div id="" v-for="n in sele_bar" @click="returngo(n.path)">
					<img :src="n.img" style="width: 48px;" />
					<p style="margin-top: 12px;">{{n.text}}</p>
				</div>
			</div>
		</div>
		<div class="H50" style="background-color: white;">

		</div>
		<div class="pc_box" v-for="(n,index) in info.list">
			<div id="" style="width: 95%;margin: auto;padding-top: 10px;">
				...
				<div class="bo_box">
					...
					<div style="display: inline-block;float: right;" v-if="islogin != null">
					...
					<div style="display: inline-block;float: right;" v-else="">
						<img src="/static/images/community_ic_collect@2x.png" style="width: 24px;margin-right: 15px;" @click="dianzhan(index,n.id,n.is_collect)" class="pic_bg1"/>
						<!--<img src="/static/images/community_ic_collect_selected@2x.png" style="width: 24px;margin-right: 15px;" @click="dianzhan1(index,n.id,n.is_collect)" class="pic_bg2" v-if="n.is_collect == 1"/>-->
						<!--<img src="/static/images/community_ic_share@2x.png"style="width: 20px;"/>-->
					</div>
				</div>
...
//					$('.pic_bg2').show()
		...
		mounted: function() {
			...
			//系统公告请求
			//修改title
			this.$store.commit('changetitle', '社区');
			//修改头部返回箭头
